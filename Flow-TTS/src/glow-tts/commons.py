...
class Adam():
  def __init__(self, params, scheduler, dim_model, n_gpu_bs, warmup_steps=4000, lr=1e0, betas=(0.9, 0.98), eps=1e-9):
    self.params = params
  ...
  def _get_lr_scale(self):
    lr_correction = (self.n_gpu_bs / self.norm_const) ** (1 / 2)
    if self.scheduler == "noam":
      step_num = self.step_num * self.n_gpu_bs / self.norm_const
      step_num = step_num if step_num != 0 else 1
      return lr_correction * np.power(self.dim_model, -0.5) * np.min([np.power(step_num, -0.5), step_num * np.power(self.warmup_steps, -1.5)])
    else:
      return lr_correction
  ...
  def load_state_dict(self, d, step_num, new_n_gpu_bs, load_n_gpu_bs):
    self._optim.load_state_dict(d)
    self.step_num = int(step_num * load_n_gpu_bs / new_n_gpu_bs)
    self.n_gpu_bs = new_n_gpu_bs
    self._scale_lr()
