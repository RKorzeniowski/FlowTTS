...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, audiopaths_and_text, hparams, add_noise=None):
        self.audiopaths_and_text = load_filepaths_and_text(audiopaths_and_text)
        self.text_cleaners = hparams.text_cleaners
        self.max_wav_value = hparams.max_wav_value
        self.sampling_rate = hparams.sampling_rate
        self.load_mel_from_disk = hparams.load_mel_from_disk
        self.add_noise = add_noise if add_noise is not None else hparams.add_noise
...
class TextMelCollate:
    ...
    def __call__(self, batch):
        ...
        if len(batch[0]) == 3:
            ...
            comb_alignment = commons.unpack_alignment(align_padded, align_lengths, div=self.n_frames_per_step)
