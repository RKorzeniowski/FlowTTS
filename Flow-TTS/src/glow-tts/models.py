...
class FlowGenerator(nn.Module):
  def __init__(self, 
    ...
    self.external_alignment = external_alignment if external_alignment is not None else False
    super().__init__()
    self.n_vocab = n_vocab
    self.hidden_channels = hidden_channels
    self.filter_channels = filter_channels
  ...
  def forward(self, gen, *args, **kwargs):
    if gen:
      result = self.generate(*args, **kwargs)
    else:
      result = self.backward_decoder(*args, **kwargs)
    return result

  def generate(self, x, x_lengths, y=None, g=None, noise_scale=1., length_scale=1., external_alignment=None, **kwargs):
    if g is not None:
      g = F.normalize(self.emb_g(g)).unsqueeze(-1)
    x_m, x_logs, logw, x_mask = self.encoder(x, x_lengths, g=g)

    if self.external_alignment:
      attn, align_mel_lengths = external_alignment
      y_mask = torch.unsqueeze(commons.sequence_mask(align_mel_lengths, None), 1).to(x_mask.dtype)
      attn = attn.cuda()
    else:
      w = torch.exp(logw) * x_mask * length_scale
      w_ceil = torch.ceil(w)
      y_lengths = torch.clamp_min(torch.sum(w_ceil, [1, 2]), 1).long()
      y, y_lengths, y_max_length = self.preprocess(y, y_lengths, None)
  ...
  def backward_decoder(self, x, x_lengths, y=None, y_lengths=None, g=None, external_alignment=None, **kwargs):
    if g is not None:
      g = F.normalize(self.emb_g(g)).unsqueeze(-1)
    x_m, x_logs, logw, x_mask = self.encoder(x, x_lengths, g=g)
